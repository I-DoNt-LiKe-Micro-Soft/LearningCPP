...
#include <iostream>
#include "Extern.cpp"  /*We must inlcude our external file using this syntax*/

static void StaticVariableExample() { 
	/*This function and varaible may not be called or used in any other files*/
	static std::string MyStaticVariable = "Hello I Am Static I Can Only Be Called Inside Of This Translation Unit (This file)";
}

int main(){

	ExternalFunctionExample();  /*We are calling a function from another file "Extern.cpp" in this case*/

	StaticVariableExample();

	std::cout << MyAge << std::endl; /*We can access the external variable located in the other file "Extern.cpp"*/
